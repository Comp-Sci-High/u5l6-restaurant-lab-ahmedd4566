...
async function startServer() {
  await mongoose.connect("mongodb+srv://SE12:CSH2026@cluster0.ytvmkmf.mongodb.net/restaurant")
...
app.post("/menu/test", async (req, res) => {
  const testItem = await new MenuItem({
    name: "Burger",
    cost: 4,
    category: "fast food",
...
app.get("/menu/value", async (req, res) => {
  const cheapItems = await MenuItem.find({ cost: { $lt: 5 } })
  res.json(cheapItems)
})
